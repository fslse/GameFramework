        ...
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
        {
            ...
            public override float ExpireTime
            {
                ...
                set
                {
                    ...
                    if (Math.Abs(ExpireTime - value) < 0.001)
                    {
                        return;
                    }
            ...
            private Object<T> GetObject(object target)
            {
                ...
                return _objectMap.TryGetValue(target, out var internalObject) ? internalObject : null;
            }
            ...
            private List<T> DefaultReleaseObjectFilterCallback(List<T> candidateObjects, int toReleaseCount, DateTime expireTime)
            {
                ...
                for (int i = 0; toReleaseCount > 0 && i < candidateObjects.Count; i++)
                {
                    ...
                    {
                        if (candidateObjects[i].Priority > candidateObjects[j].Priority || candidateObjects[i].Priority == candidateObjects[j].Priority && candidateObjects[i].LastUseTime > candidateObjects[j].LastUseTime)
                        {
